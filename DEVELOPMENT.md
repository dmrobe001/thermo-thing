...
> **Status (as built):** Implemented in equivalent velocity-impulse form. Code §08.1 integrates the applied forces to candidate velocities; code §08.3 then solves the Schur complement (code §07) for the multipliers `lambda` and applies `M^-1 J^T lambda` as a single velocity projection. This is the discrete counterpart of the acceleration-level system above: the explicit `J_dot·v` term is absorbed into that projection rather than assembled, and `beta*phi` is the Baumgarte term `sim.beta/h · C`. Baumgarte alone is only an approximate velocity projection -- its leak grows with per-substep drift and with how many rows are coupled (visibly worse on a 5-link chain than a double pendulum for the same `beta`). Code §08.6 closes that gap exactly, independent of chain length: absent a live drag, an active heat/flow interaction, or a piston doing flow work against the background at `P_bg` (all three legitimate energy sources/sinks), the mechanical+gas total entering the substep is treated as an invariant, and any post-solve discrepancy is folded back with a single uniform velocity rescale -- the same idea as a velocity-rescaling thermostat. That background flow-work term (`P_bg · dV`, code §08.5's `g._Watm`) has to be tracked explicitly alongside heat/flow's `Q`: a gas's own `dU = -P_gas · dV` only ever credits the *gas's* internal energy, while the force the piston actually feels is the *net* `(P_gas - P_bg)`, so the `P_bg · dV` difference is real work crossing into or out of the mechanism at the background boundary -- omitting it from the invariant made §08.6 silently overwrite the correct post-solve KE to defend a target that didn't balance, which showed up as the pressure-differential force never actually decelerating/reversing the piston once `P_gas` crossed `P_bg`.
>
> That per-substep rescale has one more gap of its own: it's a *multiplicative* correction (`v *= sqrt(target/actual)`), so it's only well-defined while there's actual KE to scale. Right at any island's own turning point -- a pendulum at the top of its swing, a piston at the top of its bounce -- KE passes through zero for real, and both branches of code §08.6 fall back to leaving velocities untouched for that one substep rather than divide by ~0. The gap that fallback couldn't resolve doesn't vanish on its own: the next substep's `preE` is read straight off whatever state was left standing, so an uncorrected gap becomes the new baseline permanently rather than something a later substep makes up for -- confirmed on the plain (gasless) pendulum example, whose entire slow multi-minute energy loss landed exactly on the substeps where this fallback fired, with every other substep's own delta at floating-point zero. `ENERGY_BANK` (code §08.0, keyed by an island's own body-id signature) defers that gap instead of dropping it, folding it into the very next substep where the same island's KE is large enough to actually rescale -- so the running total now stays exactly conserved across a turning point, not just between them.
>
> A *free* island has a harder version of the same problem, this time with no `ENERGY_BANK`-style fix available: its rigid (COM) motion is fully determined by momentum/angular-momentum targets that are already exact (impulse-momentum is exact for a constant force regardless of step size), which leaves *no* spare freedom in velocity to also hit the energy target -- only the internal/shape field (§08.6's `keInt`) has that freedom, since it alone carries zero net momentum. A free island with *no* internal motion at all (the default state of a piston placed fresh with the `gas` tool and no push -- both plates start at rest, and the tool sizes the gas so its initial pressure already matches the background) has nothing for that internal channel to work with, so the shortfall goes unresolved forever rather than merely at a turning point: confirmed on the simplest possible case, a single free body with gravity and nothing else at all, whose energy decreased *linearly*, unboundedly, at exactly `0.5*M*g^2*h^2` per substep -- the textbook secular drift of a symplectic-Euler position update (`x += h*v_new`) under *constant* (non-oscillating) acceleration, which the usual "bounded energy error" guarantee for symplectic integrators only covers for bounded/oscillatory motion. Code §08.6 now applies a second additive fix, this time to *position*: the island's COM is shifted (uniformly across every body, so it can't touch the internal field) to the exact trapezoidal displacement `0.5*(V0+Vt)*h` implied by the same momentum history the velocity fix already uses -- the standard leapfrog/Verlet correction for constant acceleration, applied here only to the rigid/COM part rather than switching the whole engine's integrator. It has to run *before* `post.pe` is read for that same substep's `keTarget`, or the internal-motion rescale "discovers" the same gravity-vs-position mismatch a second time and injects real energy correcting it twice (confirmed: a 20x inflation over 2000 simulated seconds on a piston with internal motion to spare, from that ordering alone).
>
> One more gap surfaced during the same investigation, this one only visible on a free island because the fixes above finally let a long run reach it uncorrected: `physics.js` §08.5's `g.T<1e-4` floor (T can't really go negative) used to just clamp `T` after the fact, with nothing else adjusted -- but the mechanical KE consequence of the dU that would have taken it there had already been delivered in full back at §08.1, so the clamp alone left `post.U` reading higher than that dU implied while KE didn't shrink to match, the same kind of gap §08.6 has to either resolve or bank. On an anchored island the ordinary multiplicative rescale absorbs it without incident; on a free island with limited internal-motion capacity to spare, and this floor binding far more often than a real turning point does (tens of thousands of times over a long run), enough of it went unresolved to add up to a slow, real secular drift of its own. The clamped-away amount is now credited to `g._Watm` -- the same channel real atmospheric flow work already uses to leave the tracked invariant -- since it left the mechanism as real KE without a matching U decrease and shouldn't sit in `keTarget` waiting to be clawed back a second time; this cut the residual drift by roughly 35x in testing, though a much smaller bounded residual remains on very long (multi-hour) runs when this floor binds at high frequency, not yet fully eliminated.
...
>
> One narrower hard stop does exist, purpose-built rather than through the general LCP path: the gas piston's minimum-volume floor (§6.1's own status note, `gasStopRow`/`physics.js` §08.3). It isn't a row in this section's bilateral solve at all -- it's a standalone elastic reflection impulse, for reasons specific to needing that stop to conserve energy exactly (an ordinary Baumgarte-driven row, the way the cable's own end-stop works, turned out to be fully inelastic on contact).
...
A gas element is a rectangular vessel that maintains its equation of state `P = nT/V` at all times (instantaneous equilibrium; `R = 1` in these abstract units). It is defined by two endpoints, exactly the `{id, off}` shape every other feature uses: `head` (a corner fixed in some body's frame, or the world, carrying the rectangle's axis direction) and `piston` (the far corner along that axis) -- the same two-corner selection a rectangular body placement uses. `piston` is either a real body -- a movable wall, making this a piston -- or `null`, in which case the rectangle's length is a fixed constant (`len`): a sealed vessel with no moving part at all, useful purely as a heat/flow node (§6.2). Either way `V = bore · length`. The placement tool always takes the real-body branch: its two-corner drag (identical to the rectangle body tool's) creates *both* `head` and `piston` as brand-new ordinary rectangular bodies -- never world-anchored (`id: null`) or a fixed-length vessel -- laid out as an upward-facing piston (axis +y, head at the box's bottom corner, piston at the top), so both start out subject to normal-body physics (gravity, collisions with other constraints) and can be picked up by any other tool (rod, spring, heat/flow interaction, pin-to-world) exactly like a body placed with the rectangle tool. `head.id === null` and `piston === null` remain valid states of the underlying data model (e.g. the bundled examples still build a world-anchored cylinder by hand), just not ones the interactive tool produces on its own.
...
`P = nT/V` diverges as `V -> 0`; no piston with finite momentum ever reaches it in continuous time, but a fixed substep can integrate straight past a volume that small before the rising pressure has had a chance to arrest it. The axial separation `x` (`bore · x = V`) therefore carries a floor, `GAS_MIN_X`, below which the vessel is treated as having bottomed out.

> **Status (as built):** `constraints.js` §06.2's `gasFrame` softens `x` to `xc = max(x, GAS_MIN_X)` purely so `P` and `V` never divide by/report zero -- a formula safety net, not what keeps the piston from actually reaching the floor. That job belongs to `gasStopRow` (§06.2b): a per-substep elastic (restitution 1) reflection impulse, activated *anticipatorily* (predicting this step's own position update from the current closing rate, since `GAS_MIN_X` is a numerical floor rather than a real object's thickness and an ordinary piston can close the whole gap in one substep), applied in `physics.js` §08.3 ahead of the main Schur solve. Earlier attempts folded this into the shared Baumgarte-driven row solve the way the cable's own end-stop works (§C.4 of `CABLE.md`) or left the floor to the pressure formula's clamp alone; both silently broke the energy invariant once a stroke reached it -- the clamp decouples `V`'s bookkeeping from the mechanism's real motion once `x` slips under it (`§08.5`'s `dU = -P·dV` sees a frozen `V` while the piston keeps moving), and a Baumgarte row pins the piston's own closing velocity at zero on contact, destroying its KE outright rather than merely approximating -- and because that KE lands at exactly zero rather than shrinking, §08.6's multiplicative rescale can't hand it back either. The standalone elastic impulse conserves this DOF's KE exactly on every contact, needs no help from §08.6, and (unlike the clamp alone) keeps `x >= GAS_MIN_X` for real, so the two ledgers never diverge.
>
> Getting that reflection's own bookkeeping right took a second pass. Position still integrates over the *entire* substep at the post-reflection velocity (§08.4 doesn't know a reflection happened partway through the force pass), so the resulting `Vnew - g._V` reads as a real volume change that isn't real gas work -- it's just the reflection's own (already energy-conserving) kinematics. §08.5 skips crediting *that* `dV` to the gas's own `dU` on a reflecting substep for exactly this reason. But `g._Watm` (the same section's atmospheric flow-work term, `P_bg · dV`) is a *different* energy channel -- real work the background does at the piston's open face, which happens regardless of why the piston moved -- and an early version of this fix zeroed it too, which silently broke the invariant a second time in the opposite direction (confirmed by instrumented play: `KE + PE + U + P_bg·V`, the true atmosphere-inclusive total, grew by exactly one `P_bg · dV` on every reflecting substep). The two now diverge deliberately: `dU` skips a reflected substep's `dV`, `g._Watm` never does.
