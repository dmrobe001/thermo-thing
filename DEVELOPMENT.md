...
| Belt (inextensible) | belting is conserved between each pair of consecutive grips | 1 per pair, see §4.1c |
...
**A rod may additionally be marked `posable`, which says nothing about the running physics and everything about editing.** A posable rod is an ordinary rigid rod at every substep. While the player *poses* the machine, though -- dragging a body around with the sim paused (`js/tools.js` §13.6) -- a posable rod **directly jointed to the body under the cursor** is **released** (`js/constraints.js` §06.2d): its distance row and both its welds go away for the duration of the drag, leaving nothing but its line. The two ends slide toward and away from each other, every body it joins turns freely, and its extra control points (§4.1b) stop being rigid attachments at a station and become *riders* held on the line and free to slide along it -- which is to say the rod becomes exactly a slot with no prismatic ends, a rail with everything on it pinned rather than welded. "Directly jointed" is `conEndpoints` and nothing cleverer: the rod names the dragged body as one of its ends or as one of its extra points, so the release reaches exactly as far as the hand does and a posable rod one joint further out stays rigid. A release also un-grounds: a posable rod welded at both ends stops freezing its far end (`SCENE.md` §S.8) for as long as the drag lasts, so the body the player is trying to slide along it can actually move. When the drag step ends, the rod re-reads its length, its welds' rest angles and its points' stations from the pose the player just produced, and is rigid again on that pose -- so a posable member is one whose geometry is an editable-by-dragging parameter, not one that fights the drag and snaps back. It is the one departure from "a constraint holds what it holds"; it is confined to the paused editor, it is opt-in per rod, and it is invisible until it happens: a posable rod nobody is dragging draws, reads and solves as the rigid rod it is. The release is described by two pieces of state, both in §06.2d: `posingRoot`, the body a pose gesture is dragging, held from pointerdown to pointerup, which is what the *canvas* draws; and `posing`, whether the rows being built right now are inside the released scope, which is what the *solver* sees. Between two pointermoves the second is zero and the rod is rigid again -- which is what makes the recapture meaningful and keeps `conMaxC`, the violation highlight and `saveState` honest.
...
**Every gripping node carries one number: its BELT DISTANCE.** Run a tape measure round the unstretched belting from an arbitrary mark and every point of the belt has a coordinate `mu`, running `0 .. restLen` and wrapping round. A tied eyelet's `mu` is exactly that — how far round the loop that body is clamped. A wheel's is the same thing carried in its own frame, so the belting at the contact is `mu + sigma*r*(psi - theta)` with `psi` the belt's heading there: turn the wheel and the belting at the contact changes, which is what no-slip means; the constant does not. Both are one expression, since `sigma*r` is zero on an eyelet.

These constants are **absolute, not gaps between neighbours**, and that is the point of them. What the belt holds is that consecutive grips are the right belting apart — but *which* grips are consecutive is not a fact about the belt, it is a fact about where everything currently is: a wheel the belting peels off stops being a grip, and a tied eyelet carried round a wheel ends up between a different pair. Stored as gaps, every one of those events means re-deriving constants that did not physically change, and the belt loses whatever it was holding. Stored as belt distances, none of them touch the stored data: the pairing is derived, the constants are not. `restLen` is then the loop's own period — how much belting there is — and is a field of the belt rather than a sum over its nodes.

**The rows are one per PAIR of consecutive grips.** Between them runs some belting, through however many untied eyelets route it on the way:
...
plus one whole `restLen` on the pair that closes the loop, since belt distance is only defined round the loop. Differentiating, a wheel's no-slip contact pins `d(mu)/dt` at its rim to `sigma*r*(dpsi/dt - omega)` and an untied eyelet pins nothing; every `dpsi/dt` then cancels against the tangent points' own drift and what is left is entirely local and velocity-linear:

```
sigma_p*r_p*omega_p - sigma_q*r_q*omega_q = sum over the pair's spans of  u . (v_next - v_this)
```

with `v` the nodes' own anchor velocities (a wheel's centre, an eyelet's point). It is a material balance: what the near wheel feeds in, less what the far wheel takes out, is what the belting between them lengthens by. Two wheels with fixed centres reduces to exactly the fixed phase ratio the belt has always held. Note what the wrapped arcs do *not* do: they never appear as an inter-node distance. A wheel's own two belt-distance terms sit in two different rows and their difference is precisely `r*alpha`, its arc — so an arc belongs to the wheel that carries it, and only reappears as length when the rows are summed round the loop, where the terms telescope and leave `L = restLen`. That is why the rest length is a field and not a row.

**Belting can only bear on a wheel from outside.** A wrap angle is a length of contact, so it cannot go negative: a negative one is the belt cutting through the disk rather than lying on it. A wheel whose wrap would come out negative has been **peeled off** — it leaves the path entirely, the belting runs straight by, it grips nothing and turns free, and it seats again when the geometry brings its wrap back positive. This is not a clamp bolted on: at the moment a wrap reaches zero the wheel's arrival and departure tangent points coincide *and* its two spans are the same tangent line, so the three points are collinear and dropping the wheel changes neither the path, its length, nor the belting held between the grips on either side. Peeling off and seating are both silent, and happen at the same geometry in both directions.

Two details make it work. The engaged set is resolved by a **fixed point** rather than one pass, since dropping one wheel moves its neighbours' tangents and can peel another; only drops are taken in a pass, so it terminates. And a peeled wheel **keeps its heading anchor** — the wrap is only negative relative to the branch that anchor picks, so clearing it would re-seed the wrap into `[0, 2pi)`, read a hair under a full turn instead of a hair below zero, and seat the wheel straight back on. While off the path the anchor is carried along by the belting passing nearest, so the two agree on which turn they are on whenever the belt comes back. Its belt distance is carried the same way and for the same reason (there is no grip point to hold while nothing is gripped), which makes `mu` something a *run* can change and so part of the Reset snapshot, alongside the cable's `spoolAngle`.

One honest caveat: a wheel that has been turning free while the belt was off it is put back on the belt's own rim speed the step it seats. That is a clutch, and a real one dissipates; this engine has no dissipation channel, so §08.6 holds the ledger flat across it instead. Brief peel-offs — the common case, a belt shaken off and back — barely show it. A separate limit is at *seeding*: with no anchor yet, a wrap can only be normalised into `[0, 2pi)`, so a scene authored with a wheel the belting already clears loads with that wheel wrapped rather than peeled. Peel-off is detected from motion thereafter.

**Softness is the reciprocal of the belting's elastic modulus**, and it is the one thing a belt has that a rigid loop does not. At `soft = 0` the segment rows above are the whole belt and the loop is inextensible. Above zero, the rows drop to the `n-1` *differences* of adjacent pairs -- which hold the no-slip ratios exactly, as before, and say that whatever the belt has stretched, it has stretched by the same amount everywhere -- and the loop's length is carried instead by a **force element** (`js/physics.js` §08.1): tension `T = max(0, L - restLen) / soft`, pulling each node's anchor along `T*(u_out - u_in)`, the gradient of the path length. Tension only: belting shorter than its rest length is slack and pushes nothing. Its strain energy `stretch^2 / (2*soft)` joins the spring potential in the ledger (§7, code §12.1), which is what keeps the per-island energy target honest about it. Note what the force does *not* do: `dL/dq` has no `theta` component, so tension alone puts no torque on any wheel. What turns a wheel is the *difference* in tension across it, and that is carried by the no-slip rows -- which is why a soft belt still transmits its ratio exactly, and why a loop with only one gripping node in it (nothing to hold a difference against) correctly drives nothing.
