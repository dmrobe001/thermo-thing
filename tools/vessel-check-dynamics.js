...
const a=0.4, gam=1.4, Patm=1.0;
const mShell=2.0, mGas=0.5;
const M=mShell+mGas;
// uniform slab in material coords: mu = M/12, A = M a^2/12
const mu=M/12, A=M*a*a/12;
const V0=a*1.0, T0=1.0; // L0=1
const kappa = (mGas*T0/V0)*Math.pow(V0,gam);  // P0*V0^gam
...
let y=[0,0,0,1.0, 0.3,-0.2, 4.0, 0.5];
const E0=E_of(y), L0=Lang_of(y);
let minL=9,maxL=0;
const h=1e-5;
for(let i=0;i<2000000;i++){ y=rk4(y,h); minL=Math.min(minL,y[3]); maxL=Math.max(maxL,y[3]); }
