...
| `posable-check.js` | a `posable` rod is a rail while it is dragged -- length free, welds off, extra points riding the line -- keeps whatever pose the drag leaves it at, re-grounds where it lands, and integrates identically to a plain rod when nothing is being dragged | `DEVELOPMENT.md` §4.1, `SCENE.md` §S.8 |
