...
| `belt-check.js` | a belt's path is the textbook two-pulley tangents and wraps (open and crossed) worked out independently here; it holds `rA*wA = +-rB*wB` exactly and its loop's total length with it; an untied eyelet routes the belt without touching the ratio while a tied one is carried by it and a welded one turned by it; softness is a modulus (tension = stretch/softness, tension-only, its strain in the ledger and the ledger flat); `posable` takes it out of the rows entirely; and the file, the reader and the click that builds one all agree | `DEVELOPMENT.md` §4.1c |
