...
| `posable-check.js` | a `posable` rod jointed to the dragged body is a rail for that drag -- length free, welds off, extra points riding the line -- keeps whatever pose the drag leaves it at and re-grounds where it lands, while one the drag never reached, or any posable rod with nothing being dragged, is the rigid rod it draws and integrates as | `DEVELOPMENT.md` §4.1, `SCENE.md` §S.8 |
...
| `belt-check.js` | a belt's path is the textbook two-pulley tangents and wraps (open and crossed) worked out independently here; it holds `rA*wA = +-rB*wB` exactly and its loop's total length with it; an untied eyelet routes the belt without touching the ratio while a tied one is carried by it and a welded one turned by it; softness is a modulus (tension = stretch/softness, tension-only, its strain in the ledger and the ledger flat); `posable` takes it out of the rows entirely; belting bears on a wheel only from outside, so a wheel it no longer reaches leaves the path continuously and seats again at the same geometry with every other node's belt distance untouched; and the file, the reader and the click that builds one all agree | `DEVELOPMENT.md` §4.1c |
| `multipoint-check.js` | a joint's extra control points do what each kind says they do: a rod's is rigid to the bar (and turns with it when locked), a slot's rides the rail and slides, a pin's joins the pivot, a rack's jointed body travels with the rack; plus the lock a new point inherits, the island an extra point couples into, and -- reaching the TOOL layer, as `posable-check.js` does -- the placement gesture that creates them | `DEVELOPMENT.md` §4.1b |

Run any of them with `node tools/<script>`.

`scene-roundtrip.js`, `rack-check.js`, `multipoint-check.js`, `posable-check.js`, `select-check.js`, `belt-check.js` and `expr-check.js` are the exceptions to the "do not import the simulator" rule
