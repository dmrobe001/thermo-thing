...
- `constraints` -- array of **couplings**: everything that produces solver ROWS, as opposed to `bodies`, which is everything that produces COLUMNS. Each carries `type` and transient solver outputs (`_lam`, `_rows`, `_roles` -- what each of those rows *is*, so §09.3 can look a multiplier up by name). Two of the kinds are the scene model's spine and neither has the `a`/`b` endpoints the others do:
  - a **vertex** (`type:'vertex'`, §06.2e) is a named point carrying its own place (`at`) and a list of **incidences**, one per body it touches, each saying where the vertex sits in that body's frame and whether it is `join`ed and `weld`ed there -- and, for a line, whether it may `slide`. Where an incidence locates it, that relation says where it is; where none does, `at` does, so a vertex always has a position and **outlives every body it ever touched**. What the *panels* list is wider than that and is not stored anywhere: every body whose **extent** covers the point, joined or not, with `joined` as the only control either list needs (`VERTEX.md` §X.11). It compiles to two rows per joined incidence past the primary and one per welded incidence past the first. It replaced the pin.
  ...
  Both live here because a coupling is what they are, which is why islands, the row assembly, the position projection, Reset and the selection's membership rule work on them with no per-kind branching.

  **A disk, a vertex and a line are three objects of equal standing.** A constraint between them is a statement *about* them and never a claim on their existence, so **no deletion cascades**: taking a body out removes the incidences that named it and nothing else (`dropBodyFromConstraints`, §06.2b), and taking a line out removes its joints' incidences and nothing else (`deleteConstraint`, the one path every delete goes through). An object left under-determined -- a vertex nothing holds, a line with fewer than two placed joints -- is in a *state*, not on its way out: it keeps its identity, its place and its label, and simply has no rows for as long as that lasts. Anything that made one object's existence conditional on another's would be the body-owns-its-joints model this one replaced. See `VERTEX.md`. `conEndpoints(con)` is the one answer to "which bodies does this couple". A line may also carry `posable`, the one field that says nothing about the running physics: it releases the bar to a bare rail while the player drags a body it touches, with the sim paused (`constraints.js` §06.2d).
