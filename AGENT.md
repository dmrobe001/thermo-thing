...
| `js/tools.js` | §13 | `TOOLS` (incl. the vertex and line tools and the heat/mass interaction tools), `setTool`/`setLineMode`, `pickBody`, `pickVertexAt`, `joinVertexToLine`, `pickVessel`, `pickInteraction`, `dropInteractionsOn`, `snapAnchor`, §13.3 `conHandles`/`applyHandle` (the vertex handle: the point moves and every station on every line it joins is re-read, the hand setting the bar), §13.6 `poseDragTo` (which is the only place that sets one, so a drag on a body a line GROUNDS moves nothing -- it is pinned), pointer handlers |
| `js/inspector.js` | §14 | §14.0 `numRow`/`numVal`/`wireNumIns` (one editable number: arithmetic in, a number committed, the arrow keys stepping the value); `clearSelection`, `select*`, `renderInspector` (incl. the interaction panel, the vertex panel and the vertex list a body shows), §14.2b `renderVesselInspector`, §14.2c the incidence row (`incidenceRow`/`incidenceRowsFrom`/`wireIncidenceRows`/`setIncidenceJoin`/`commitIncidenceOff`/`commitIncidenceStation`) -- the one row a vertex's panel, a body's and a line's are all built out of, §14.3 `updateInspectorLive`/`updateIncidenceLive` (every row a moved vertex touches, refreshed each frame; a row that has appeared or gone rebuilds the panel) |
