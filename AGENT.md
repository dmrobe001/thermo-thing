...
- `constraints` -- array of typed joint objects; each carries `type`, endpoint refs (`a`, `b`), type-specific parameters, and transient solver outputs (`_lam`, `_rows`). A pin, rod, slot or rack may also carry `pts` -- its **extra control points**, the third and further ends attached to the same joint (`constraints.js` §06.2c). A **belt** is the one kind with no `a`/`b` at all: it is a closed loop, so *every* node of it -- a wheel it wraps or an eyelet it passes through -- is one of those points, in the order the belting runs through them, each gripping node carrying its BELT DISTANCE -- how far round the belting it holds, against the loop's own `restLen` (`constraints.js` §06.2e). `conEndpoints(con)` is the one answer to "which bodies does this couple". A rod or a belt may also carry `posable`, the one joint field that says nothing about the running physics: with the sim paused, it releases the rod to a bare rail while the player drags a body the rod names, and takes the belt out of the way entirely for any pose drag at all -- a released belt is an absence rather than a rail, so its reach need not match the gesture's (`constraints.js` §06.2d).
...
- **Every kind of scene object has exactly one constructor** (`makeBody`,
  ...
  `makeCvtCon`, `makeRackCon`, `makeKnifeCon`, `makeCableCon`,
  `makeSpringCon`, `makeRotSpringCon` in §06.1/§06.2, `makeBeltCon` in §06.2e,
  `makeConPoint` in §06.2c (which builds a belt's nodes too);
...
| `js/constraints.js` | §06 | `bodyIndex`, `epWorld`, `epFrame` (endpoint velocity columns, incl. a vessel's length column), `twoPointFrame`, `cableFrame`, `rowsFor`, §06.2b `recaptureConAngles`/`recaptureConPose` (re-read what a line joint holds off the live geometry), §06.2c extra control points (`conPoints`, `makeConPoint`, `linePointRows`, `conEndpoints`), §06.2d posable joints (`beginPosing`/`endPosing`, `withPosing`, `conPosing`/`conReleased`, `recapturePosable`), §06.2e the belt (`makeBeltCon`, `beltFrame` incl. which wheels the belting is still on, `beltMu`/`beltPairs`/`beltRows`, `beltTension`, `beltRefresh`, `beltSettle`, `beltSetRestLen`), `makeSpringCon`, `makeRotSpringCon`, `rotSpringSpiralGeom` |
| `js/solver.js` | §07 | `solveLinear` -- dense Gauss-Jordan on the Schur complement |
| `js/physics.js` | §08 | `substep` -- §08.0b `vesselExchangeStep` (heat & mass, at frozen geometry, ahead of everything) -> forces (gravity, drag, springs, belt tension, vessel centrifugal) -> §08.1b `vesselGasStep` -> constraint solve -> position integration -> energy-conservation rescale |
...
| `js/render.js` | §11 | `render` orchestrator; `drawBody`, `drawVessel`, `drawConstraint` (incl. `drawBeltConstraint`, which draws the belting itself off `beltFrame`), `drawCable`, `drawSpring`, `drawRotSpring`, §11.4c `drawInteraction`, `drawReaction`, ... |
...
| `js/tools.js` | §13 | `TOOLS` (incl. the heat/mass interaction tools), `setTool`, `pickBody`, `pickVessel`, `pickInteraction`, `dropInteractionsOn`, `snapAnchor`, `beltNearestSpan`/`beltNodeAt`/`addBeltNode` (what a click makes of a belt), `conHandles`, pointer handlers |
| `js/inspector.js` | §14 | §14.0 `numRow`/`numVal`/`wireNumIns` (one editable number: arithmetic in, a number committed, the arrow keys stepping the value); `clearSelection`, `select*`, `renderInspector` (incl. the interaction panel), §14.2b `renderVesselInspector`, §14.2c `beltNodesCard` (the belt's loop, in order), `updateInspectorLive` |
