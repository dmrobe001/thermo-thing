...
| `js/tools.js` | §13 | `TOOLS` (incl. the vertex and line tools and the heat/mass interaction tools), `setTool`/`setLineMode`, `pickBody`, `pickVertexAt`, `joinVertexToLine`, `pickVessel`, `pickInteraction`, `dropInteractionsOn`, `snapAnchor`, §13.3 `conHandles`/`applyHandle` (the vertex handle: the point moves and every station on every line it joins is re-read, the hand setting the bar), pointer handlers |
