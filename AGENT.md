...
- `constraints` -- array of typed joint objects; each carries `type`, endpoint refs (`a`, `b`), type-specific parameters, and transient solver outputs (`_lam`, `_rows`, `_roles` -- what each of those rows *is*, so §09.3 can look a multiplier up by name). A **vertex** (`type:'vertex'`, `constraints.js` §06.2e) lives here too and is the one kind with no `a`/`b`: it is a named point carrying a list of **incidences**, one per body it touches, each saying where the vertex sits in that body's frame and whether it is `join`ed and `weld`ed there. It compiles to the rows the retired `pin` built -- two per joined incidence past the primary, one per welded incidence past the first -- and because it is a coupling in the couplings list, islands, the row assembly, Reset and the selection's membership rule all work on it unchanged. See `VERTEX.md`. A pin, rod, slot or rack may also carry `pts` -- its **extra control points**, the third and further ends attached to the same joint (`constraints.js` §06.2c). `conEndpoints(con)` is the one answer to "which bodies does this couple". A rod may also carry `posable`, the one joint field that says nothing about the running physics: it releases the rod to a bare rail while the player drags a body the rod is jointed to, with the sim paused (`constraints.js` §06.2d).
...
- **Every kind of scene object has exactly one constructor** (`makeBody`,
  `makeRectBody`, `makeVessel` in §05.2; `makeRodCon`, `makeSlotCon`,
  ...
  `makeVertex`/`makeVertexOn` in §06.2e; `makeInteraction` in §17.1),
...
| `js/constraints.js` | §06 | `bodyIndex`, `epWorld`, `epFrame` (endpoint velocity columns, incl. a vessel's length column), `twoPointFrame`, §06.1b `lineFrameOf`/`frameAngleRow` (a straight bar's own frame -- the third frame kind beside a body's and the background's, owning no coordinates), §06.2e the vertex (`makeVertex`/`makeVertexOn`/`vertexWorld`/`verticesOn`), `cableFrame`, `rowsFor` (every row tagged with its `role`), §06.2b `recaptureConAngles`/`recaptureConPose` (re-read what a line joint holds off the live geometry), §06.2c extra control points (`conPoints`, `makeConPoint`, `linePointRows`, `conEndpoints`), §06.2d posable rods (`beginPosing`/`endPosing`, `withPosing`, `rodPosing`/`rodReleased`, `recapturePosable`), `makeSpringCon`, `makeRotSpringCon`, `rotSpringSpiralGeom` |
...
| `js/inspector.js` | §14 | §14.0 `numRow`/`numVal`/`wireNumIns` (one editable number: arithmetic in, a number committed, the arrow keys stepping the value); `clearSelection`, `select*`, `renderInspector` (incl. the interaction panel, the vertex panel and the vertex list a body shows), §14.2b `renderVesselInspector`, `updateInspectorLive` |
