...
| `js/constraints.js` | §06 | `bodyIndex`, `epWorld`, `epFrame` (endpoint velocity columns, incl. a vessel's length column), `twoPointFrame`, `cableFrame`, `rowsFor`, §06.2b `recaptureConAngles`/`recaptureConPose` (re-read what a line joint holds off the live geometry), §06.2c extra control points (`conPoints`, `makeConPoint`, `linePointRows`, `conEndpoints`), §06.2d posable joints (`beginPosing`/`endPosing`, `withPosing`, `conPosing`/`conReleased`, `recapturePosable`), §06.2e the belt (`makeBeltCon`, `beltFrame`, `beltSegments`, `beltRows`, `beltTension`, `beltRefresh`, `beltSetRestLen`), `makeSpringCon`, `makeRotSpringCon`, `rotSpringSpiralGeom` |
