...
- **The feature-first editing model** -- the editor is tool-first, with no persistent named features or relation menu (DEVELOPMENT.md §8). Bulk editing has since landed on top of it: a lasso selection with a transform box, and a stash of reusable widgets held as scene fragments (`SCENE.md` §S.9). A plan to close it is written up in `VERTEX.md`: vertices become first-class named points, and rod, slot, rack and spring collapse into one `line` body carrying them.
